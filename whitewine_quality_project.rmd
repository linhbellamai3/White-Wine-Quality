...
p5 <- qplot(x=residual.sugar, data = ww, binwidth =0.05)+
...
The histogram for residual sugar is skewed to the left significantly with most
of white wines have density less than 2 g/dm^3. The log10 plot of residual sugar 
shows that residual is dual-modal. For the density, most of the white wines have density of 
...
I want to see the boxplots for all the attributes because I saw a lot of 
imbalance and skewness in their histograms.

```{r echo=FALSE, Univariate_Plot}
subset(ww, select = -c(X)) %>%
  keep(is.numeric) %>%
  gather() %>%
...
I can see that most of the variables have tails of outliers, except for alcohol. Most of the outliers stay on the higher value sides, which explains why most of the histograms are skewed to the left side. Chlorides has the worst case of outliers, with the biggest outlier to be about 10 times of its IQR. Density, fixed acidity, free sulfur dioxide and residual sugar all have a significant outliers as their maximum, which may affect their correlation with other variables. 
...
Because X is just the ID for each wines, I decide to exclude it from the correlation plot between quality and other variables. I pick Spearman correlation since quality is an ordinal variable.

```{r Correlation_coeff, echo=FALSE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
wine_cor_data <- subset(ww, select = -c(X,sugar.bucket,pH.bucket,alcohol.bucket))
ggcorr(wine_cor_data,
...
Here are the scatterplot to present the relationship between each variables:

```{r Correlation_Pairs, echo=FALSE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
cor_plot <- ggscatmat(subset(wine_cor_data, select = -c(quality))) + theme_grey(base_size = 5)
wine_cor_data.m <- melt(wine_cor_data, id.vars = 'quality',
                        measure.vars = c('fixed.acidity',
                                         'volatile.acidity',
                                         'citric.acid',
                                         'residual.sugar',
                                         'chlorides','free.sulfur.dioxide', 'total.sulfur.dioxide', 'density', 'pH', 'sulphates', 'alcohol'))
qual_plot <- ggplot(wine_cor_data.m, aes(x=factor(quality), y=value))+
  geom_boxplot()+
  facet_wrap(~variable, scales = "free")+
  xlab("Quality")
grid.arrange(cor_plot, qual_plot, ncol = 1)
```
...
The plot and summary shows that the

Now I want to see alcohol and quality relationship.

```{r echo=FALSE, Bivariate_Plot11}
ggplot(aes(x=factor(quality), y=alcohol), data=ww)+
...
From the graph, it looks like both the mean and mode of alcohol go down for quality 3 to 5 and then increase after that. 

so I will explore their relationship another way, using boxplot between alcohol bucket and quality
...
Based on the plots, only medium-high-alcohol-content white wines shows a trend
that as higher alcohol has higer quality. However, the other catergories don't 
show any significant trend like Medium-High categories

Now I want to see the relationship between residual sugar and quality.

```{r echo=FALSE, Bivariate_Plot17}
ggplot(aes(x=factor(quality), y=residual.sugar), data=ww)+
  geom_jitter(alpha = 1/10)+
  geom_boxplot(alpha = 1/10, color = 'blue')+
  stat_summary(fun.y='mean', geom = 'point', color = 'red')+
  labs(x = 'Quality (score between 3 and 9)',
...
The graph shows that there are no relationship between residual sugar 
and quality. Let's explore their relationship with boxplot with sugar bucket.
...
ggplot(aes(x=factor(quality), volatile.acidity), data=ww)+
  geom_jitter(alpha = 1/10)+
  geom_boxplot(alpha = 1/10, color = 'blue')+
  stat_summary(fun.y='mean', geom = 'point', color = 'red')+
  labs(x = 'Quality (score between 3 and 9)',
       y = 'Acetic Acid (g / dm^3)',
       title = 'Boxplot of Acetic Acid across qualities')
```
...
chlorides, or density.However, density has a really similar trend as alcohol.
The data was too clustered around quality of 5-6 due to the unbalanced 
...
ggplot(aes(x=density, y = alcohol, color = quality.bucket), data = ww)+
  ...
  scale_color_brewer(type = 'seq')+
  theme_dark()+
  labs(title = "Relationship of Density vs Alcohol with colored quality levels", 
